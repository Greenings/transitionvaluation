...
def make_calib_cell(p: dict) -> str:
    ...
    lines = [
        ...
        "    from assess import VF_FILE_SPECS, _resolve_wifor_dir, _load_vf_coeff, _load_gva_per_hour, REGION_TO_ISO3, PROJECT_SECTOR_TO_NACE",
        f"    _sec  = '{p['sector']}'",
        f"    _reg  = '{p['region']}'",
        ...
        "        # GVA per labour-hour: living-wage proxy from raw WifOR input",
        "        _c_gva = _load_gva_per_hour(_VF_DIR, _iso3, _nace, YEAR_CAL)",
        "        _gva_row = (f'<tr style=\"background:#f0fff0\">'",
        "                    f'<td><b>GVA / labour-hour</b><br>'",
        "                    f'<span style=\"font-size:10px;color:#555\">Living-wage proxy: GVA/h &ge; Anker LW</span></td>'",
        "                    f'<td>{_iso3}/{_nace}</td>'",
        "                    f'<td>{_c_gva:.4f} USD/h</td>'",
        "                    f'<td>${_c_gva:.2f}/FTE-hr</td></tr>')",
        "        _vf_rows.append(_gva_row)",
        ...
        "                    '<th>Coefficient</th><th>Per-unit equiv</th></tr>'",
        "                    + ''.join(_vf_rows) + '</table>'",
        "                    '<p style=\"font-size:11px;color:#888\">GVA/h source: "
        "input_data/220529_training_value_per_hour_bysector.h5 "
        "(value_per_hour_GVA_2020USD_PPP &times; MonTrain growth index)</p>')",
